...
int main(int argc, char** argv) {
    ...
    if ((fileDescriptor = shm_open("/ringmem", O_RDWR, S_IRUSR | S_IWUSR)) == -1)// открываем память под рингмем
        perror("shm_open");
    if ((ring = mmap(NULL, sizeof(Ring), PROT_READ | PROT_WRITE, MAP_SHARED, fileDescriptor, 0)) == (void*)-1)
        perror("Mmap");// получаем указатель из этой области на очередь
    ...
    munmap(ring, sizeof(Ring));// освобождаем ресурсы

...
void* producer(void* arg) {// ждет семафора fill_sem
    int thread_id = *(int*)arg;
    while (1) {
        sem_wait(&fill_sem);// когда дождался счетчик уменьшается
        pthread_mutex_lock(&ring_mutex);//лочим мьютекс для входа в критическую секцию
...
void create_producer() {
    if (num_prod_threads < max) {// не вызывает исполняемый файл а создает поток
